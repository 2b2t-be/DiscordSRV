...
public class DiscordChatListener extends ListenerAdapter {
  ...
  public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
    ...
    if (event.getAuthor() == null || event.getMember() == null || event.getAuthor().getId() == null
        || DiscordUtil.getJda() == null || DiscordUtil.getJda().getSelfUser() == null
        || DiscordUtil.getJda().getSelfUser().getId() == null
    ...
    for (Map.Entry<String, String> entry : DiscordSRV.getPlugin().getResponses().entrySet()) {
      if (event.getMessage().getContentRaw().toLowerCase()
          .startsWith(entry.getKey().toLowerCase())) {
        String discordMessage = entry.getValue();
        if (PluginUtil.pluginHookIsEnabled("placeholderapi"))
          discordMessage =
              me.clip.placeholderapi.PlaceholderAPI.setPlaceholders(null, discordMessage);
    ...
    if (DiscordSRV.getPlugin()
        .getDestinationGameChannelNameForTextChannel(event.getChannel()) == null)
      return;

    // sanity & intention checks
    String message = DiscordSRV.config().getBoolean("Experiment_MCDiscordReserializer")
      ...
      if (!hasLinkedAccount && !event.getAuthor().isBot()) {
        event.getAuthor().openPrivateChannel()
            .queue(privateChannel -> privateChannel
                .sendMessage(LangUtil.InternalMessage.LINKED_ACCOUNT_REQUIRED.toString()
    ...
    if (DiscordSRV.config().getBoolean("DiscordChatChannelBlockBots") && event.getAuthor().isBot()
        && !(event.getAuthor().getDiscriminator().equals("0000"))) {
      // debug
      DiscordSRV.info("debug: tag = " + event.getAuthor().getDiscriminator());
      DiscordSRV.info("debug: Received Discord message from bot " + event.getAuthor()
          + " but DiscordChatChannelBlockBots is on");
      return;
    }

    // blocked ids
    if (DiscordSRV.config().getStringList("DiscordChatChannelBlockedIds")
        .contains(event.getAuthor().getId())) {
      DiscordSRV.debug("Received Discord message from user " + event.getAuthor()
          + " but they are on the DiscordChatChannelBlockedIds list");
      return;
    }

    DiscordGuildMessagePreProcessEvent preEvent =
        (DiscordGuildMessagePreProcessEvent) DiscordSRV.api
    ...
    List<String> discordRolesSelection =
        DiscordSRV.config().getStringList("DiscordChatChannelRolesSelection");
    ...
    if (event.getMessage().getAttachments().size() > 0) {
      for (Message.Attachment attachment : event.getMessage().getAttachments().subList(0,
          event.getMessage().getAttachments().size() > 3 ? 3
              : event.getMessage().getAttachments().size())) {

        // get the correct format message
        String placedMessage =
            !selectedRoles.isEmpty() ? LangUtil.Message.CHAT_TO_MINECRAFT.toString()
                : LangUtil.Message.CHAT_TO_MINECRAFT_NO_ROLE.toString();

        placedMessage = ChatColor.translateAlternateColorCodes('&',
            placedMessage.replace("%message%", attachment.getUrl())
                .replace("%username%", DiscordUtil.strip(event.getMember().getEffectiveName()))
                .replace("%toprole%",
                    DiscordUtil.getRoleName(!selectedRoles.isEmpty() ? selectedRoles.get(0) : null))
                .replace("%toproleinitial%",
                    !selectedRoles.isEmpty()
                        ? DiscordUtil.getRoleName(selectedRoles.get(0)).substring(0, 1)
                        : "")
                .replace("%toprolecolor%",
                    DiscordUtil.convertRoleToMinecraftColor(
                        !selectedRoles.isEmpty() ? selectedRoles.get(0) : null))
        ...
        if (DiscordSRV.config().getBoolean("DiscordChatChannelBroadcastDiscordMessagesToConsole"))
          DiscordSRV.info(LangUtil.InternalMessage.CHAT + ": "
              + DiscordUtil.strip(placedMessage.replace("»", ">")));
      }
    ...
    for (String phrase : DiscordSRV.config().getStringList("DiscordChatChannelBlockedPhrases")) {
      if (StringUtils.isEmpty(phrase))
        continue; // don't want to block every message from sending
      if (event.getMessage().getContentDisplay().contains(phrase)) {
        DiscordSRV.debug("Received message from Discord that contained a block phrase (" + phrase
    ...
    if (message.length() > DiscordSRV.config().getInt("DiscordChatChannelTruncateLength")) {
      event.getMessage().addReaction("\uD83D\uDCAC")
          .queue(v -> event.getMessage().addReaction("❗").queue());
      message =
          message.substring(0, DiscordSRV.config().getInt("DiscordChatChannelTruncateLength"));
    }

    // get the correct format message
    String formatMessage = !selectedRoles.isEmpty() ? LangUtil.Message.CHAT_TO_MINECRAFT.toString()
        : LangUtil.Message.CHAT_TO_MINECRAFT_NO_ROLE.toString();

    // strip colors if role doesn't have permission
    List<String> rolesAllowedToColor =
        DiscordSRV.config().getStringList("DiscordChatChannelRolesAllowedToUseColorCodesInChat");
    ...
    formatMessage = formatMessage.replace("%channelname%", event.getChannel().getName())
        .replace("%message%", message != null ? message : "<blank message>")
        .replace("%username%", DiscordUtil.strip(event.getMember().getEffectiveName()))
        .replace("%toprole%",
            DiscordUtil.getRoleName(!selectedRoles.isEmpty() ? selectedRoles.get(0) : null))
        .replace("%toproleinitial%",
            !selectedRoles.isEmpty() ? DiscordUtil.getRoleName(selectedRoles.get(0)).substring(0, 1)
                : "")
        .replace("%toprolecolor%",
            DiscordUtil.convertRoleToMinecraftColor(
                !selectedRoles.isEmpty() ? selectedRoles.get(0) : null))
        .replace("%allroles%", DiscordUtil.getFormattedRoles(selectedRoles)).replace("\\~", "~") // get rid of badly escaped characters
        .replace("\\*", "") // get rid of badly escaped characters
        .replace("\\_", "_"); // get rid of badly escaped characters
    ...
    DiscordGuildMessagePostProcessEvent postEvent =
        (DiscordGuildMessagePostProcessEvent) DiscordSRV.api.callEvent(
            new DiscordGuildMessagePostProcessEvent(event, preEvent.isCancelled(), formatMessage));
    ...
    DiscordSRV.getPlugin().broadcastMessageToMinecraftServer(
        DiscordSRV.getPlugin().getDestinationGameChannelNameForTextChannel(event.getChannel()),
        postEvent.getProcessedMessage(), event.getAuthor());

    if (DiscordSRV.config().getBoolean("DiscordChatChannelBroadcastDiscordMessagesToConsole")) {
      DiscordSRV.info(LangUtil.InternalMessage.CHAT + ": "
          + DiscordUtil.strip(postEvent.getProcessedMessage().replace("»", ">")));
    }
  }
  ...
  private boolean processPlayerListCommand(GuildMessageReceivedEvent event, String message) {
    ...
    if (PlayerUtil.getOnlinePlayers(true).size() == 0) {
      DiscordUtil.sendMessage(event.getChannel(),
          LangUtil.Message.PLAYER_LIST_COMMAND_NO_PLAYERS.toString(),
      ...
      StringJoiner players =
          new StringJoiner(LangUtil.Message.PLAYER_LIST_COMMAND_ALL_PLAYERS_SEPARATOR.toString());
      for (Player player : PlayerUtil.getOnlinePlayers(true)) {
        ...
        if (hasGoodGroup)
          userPrimaryGroup =
              userPrimaryGroup.substring(0, 1).toUpperCase() + userPrimaryGroup.substring(1);
        ...
        String playerFormat = LangUtil.Message.PLAYER_LIST_COMMAND_PLAYER.toString()
            ...
            .replace("%world%", player.getWorld().getName()).replace("%worldalias%",
                DiscordUtil.strip(MultiverseCoreHook.getWorldAlias(player.getWorld().getName())));

        // use PlaceholderAPI if available
        if (PluginUtil.pluginHookIsEnabled("placeholderapi"))
          playerFormat =
              me.clip.placeholderapi.PlaceholderAPI.setPlaceholders(player, playerFormat);
  ...
  private boolean processConsoleCommand(GuildMessageReceivedEvent event, String message) {
    ...
    if (parts.length < 2)
      return false;
    if (!parts[0]
        .equalsIgnoreCase(DiscordSRV.config().getString("DiscordChatChannelConsoleCommandPrefix")))
      return false;

    // check if user has a role able to use this
    Set<String> rolesAllowedToConsole = new HashSet<>();
    rolesAllowedToConsole
        .addAll(DiscordSRV.config().getStringList("DiscordChatChannelConsoleCommandRolesAllowed"));
    rolesAllowedToConsole.addAll(
        DiscordSRV.config().getStringList("DiscordChatChannelConsoleCommandWhitelistBypassRoles"));
    ...
    if (!allowed) {
      ...
      if (DiscordSRV.config().getBoolean("DiscordChatChannelConsoleCommandNotifyErrors"))
        DiscordUtil.privateMessage(event.getAuthor(),
            LangUtil.Message.CHAT_CHANNEL_COMMAND_ERROR.toString()
    ...
    for (String roleName : DiscordSRV.config()
      ...
      boolean isAble =
          DiscordUtil.memberHasRole(event.getMember(), Collections.singleton(roleName));
    ...
    if (canBypass) {
      ...
      List<String> commandsToCheck =
          DiscordSRV.config().getStringList("DiscordChatChannelConsoleCommandWhitelist");
    ...
    try {
      ...
      DiscordSRV.error(LangUtil.InternalMessage.ERROR_LOGGING_CONSOLE_ACTION + " "
          + DiscordSRV.config().getString("DiscordConsoleChannelUsageLog") + ": " + e.getMessage());
    ...
    Bukkit.getScheduler().runTask(DiscordSRV.getPlugin(), () -> Bukkit.getServer().dispatchCommand(
        new SingleCommandSender(event, Bukkit.getServer().getConsoleSender()), parts[1]));
